...
/*
    ...
    first node in the stack and all others nodes are linked using the next pointer in the pointer part of the node therefore every
...
void freeStack(myStack * head)    {
    myStack * curNode = head;           // Set curNode to point where head node points.

    // Traverse the stack to free all the nodes once the program ends as there is no need to keep the stack in memory.
